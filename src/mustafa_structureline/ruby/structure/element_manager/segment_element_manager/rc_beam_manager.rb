require_relative '../base_element_manager'

module Mustafa
  module StructureLine
    module Structure
      module ElementManager
        module SegmentElementManager
          class RcBeamManager < BaseElementManager
            include SegmentElementManager
            include Element::SegmentElement

            def initialize(name, standard, width, height)
              super(name, standard)
              @width = width
              @height = height
              @preview_element = RcBeam.new(manager: self,
                                            start_node: Node.new,
                                            start_offset: Vector3d.new,
                                            end_node: Node.new,
                                            end_offset: Vector3d.new,
                                            style: nil,
                                            group: nil,
                                            id: nil,
                                            width: @width,
                                            height: @height,
                                            presentation: nil)
            end

            def draw_preview(view, preview_point)
              return if segment_element_preview_cannot_be_drawn?(preview_point)
              @preview_element.start_node.position = self.segments.last_segment.end_position.offset(get_start_offset)
              @preview_element.end_node.position = preview_point.offset(get_end_offset)
              @preview_element.update_drawing_points!
              @preview_element.draw(view)
            end

            private

            def register_element_from_group(group, hash)
              register_rc_beam(start_node: get_node_from_json(hash['start_node']),
                               start_offset: get_3d_vector_from_json(hash['start_offset']),
                               end_node: get_node_from_json(hash['end_node']),
                               end_offset: get_3d_vector_from_json(hash['end_offset']),
                               style: hash['style'],
                               group: group,
                               id: hash['id'],
                               width: hash['width'].mm,
                               height: hash['height'].mm,
                               presentation: hash['presentation']
              )

            end

            def register_rc_beam(start_node:, start_offset:, end_node:, end_offset:,
                                 style:, group:, id:, width:, height:, presentation:)
              @elements[id] = RcBeam.new(manager: self,
                                         start_node: start_node,
                                         start_offset: start_offset,
                                         end_node: end_node,
                                         end_offset: end_offset,
                                         style: style,
                                         group: group,
                                         id: id,
                                         width: width,
                                         height: height,
                                         presentation: presentation
              )
            end



          end
        end
      end
    end
  end
end